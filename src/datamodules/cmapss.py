...
class CMAPSSDataModule(RulDataModule):
    ...
    def __init__(self, data_dir: str, fd: int, max_rul: int, batch_size: int) -> None:
        # change data download destination from ~/.rul_datasets to project
        # print(rul_datasets.reader.data_root._DATA_ROOT)

        super().__init__(reader=CmapssReader(fd=fd, max_rul=max_rul), batch_size=batch_size)  # type: ignore
